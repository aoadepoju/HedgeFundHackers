...
// Declare instance variables here
    ...
    public Stock(String stockName, String stockSymbol, String stockDescript, String stockExch, double currPrice,
                 double openPrice, double closePrice, double yearHigh, double yearLow, double dayChange,
                 double ytdChange, double profitLoss, String stockSector) {
...
// Accessors and mutators for stock object traits.
    ...
    public String getStockDescript() {
        ...
        }//asccessor for stock description
    ...
    public double getCurrPrice() {
        return Double.parseDouble(df.format(currPrice));    //return currPrice;
        }//accessor for current price
    public void setCurrPrice(double newCurrPrice) {
        this.currPrice =
        Double.parseDouble(df.format(newCurrPrice));
        }//mutator for current price

    //OPEN PRICE ATTRIBUTE
    public double getOpenPrice() {
        return Double.parseDouble(df.format(openPrice));    //return openPrice;
        }//accessor for open price
    public void setOpenPrice(String newOpenPrice) {
        this.openPrice =
        Double.parseDouble(df.format(newOpenPrice));
        }//mutator for open price

    //CLOSE PRICE ATTRIBUTE
    public double getClosePrice() {
        return Double.parseDouble(df.format(closePrice));   //return closePrice;
        }//accessor for close price
    public void setClosePrice(double newClosePrice) {
        this.closePrice =
        Double.parseDouble(df.format(newClosePrice));
        }//mutator for close price

    //YEAR HIGH ATTRIBUTE
    public double getYearHigh() {
        return Double.parseDouble(df.format(yearHigh));     //return yearHigh;
    ...
    public double getYearLow() {
        return Double.parseDouble(df.format(yearLow));      //return yearLow;
    ...
    public double getDayChange() {
        return Double.parseDouble(df.format(dayChange));    //return dayChange;
        }//accessor for day change
    public void setDayChange(double newDayChange) {
        this.dayChange =
        Double.parseDouble(df.format(newDayChange));
        }//mutator for day change

    //YEAR-TO-DATE CHANGE ATTRIBUTE
    public double getYtdChange() {
        return Double.parseDouble(df.format(ytdChange));    //return ytdChange;
        }//accessor for ytd change
    public void setYtdChange(double newYtdChange) {
        this.ytdChange =
        Double.parseDouble(df.format(newYtdChange));
        }//mutator for ytd change

    //PROFIT LOSS ATTRIBUTE
    public double getProfitLoss() {
        return Double.parseDouble(df.format(profitLoss));   //return profitLoss
        }//accessor for profit loss
    public void setProfitLoss(double newProfitLoss) {
        this.profitLoss =
        Double.parseDouble(df.format(newProfitLoss));
        }//mutator for profit loss

    //STOCK SECTOR ATTRIBUTE
    public String getStockSector() {                        // TODO Auto-generated method stub(should this be removed?)
        return stockSector;
    ...
    public String toString() {
        return
        String.format("%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s", stockName,
                stockSymbol, stockDescript, stockExch, currPrice, openPrice, closePrice, yearHigh, yearLow,
                dayChange, ytdChange, profitLoss, stockSector);
        }
}
